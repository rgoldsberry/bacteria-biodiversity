...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    // 1 & 2. Create a variable that filters the metadata array and holds first sample.
    let metadata = data.metadata;
    let metadataFiltered = metadata.filter(Obj => Obj.id == sample)[0];
    // console.log(metadataFiltered);

    // 3. Create a variable that holds the washing frequency.
    let sampleWfreq = parseFloat(metadataFiltered.wfreq)
